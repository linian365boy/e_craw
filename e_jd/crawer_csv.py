...
class JDPhone(object):
    ...
    def set_title(self):
        ...
        try:
            with open("{}_{}.csv".format(self.key, self.T), 'w', encoding='utf-8', newline='') as f:
                f_csv = csv.DictWriter(f, self.title_list)
                f_csv.writeheader()
                f_csv.writerow(dict(zip(self.title_list, titles)))
                print("success to write csv header!")
        except Exception as e:
            print("fail to write csv header %s" % e)
    ...
    def save_info(self, *args):
        ...
        print(item)
        try:
            with open("{}_{}.csv".format(self.key, self.T), 'a+', encoding='utf8', newline='') as f:
                f_csv = csv.DictWriter(f, self.title_list)
                f_csv.writerow(item)
        except:
            pass
